...
class GaussianPolicy(nn.Module):
    ...
    def compute_power(self, action):
        # Normalize the power
        G_real = action[:, :self.M * self.K].cpu().data.numpy()
        G_imag = action[:, self.M * self.K:2 * self.M * self.K].cpu().data.numpy()
    ...
    def compute_phase(self, action):
        ...
        Phi1_real = action[:, -4 * self.L:-3 * self.L].detach()
        Phi1_imag = action[:, -3 * self.L:-2 * self.L].detach()

        Phi2_real = action[:, -2 * self.L:-self.L].detach()
        Phi2_imag = action[:, -self.L:].detach()

        return (torch.sum(torch.abs(Phi1_real), dim=1).reshape(-1, 1) * np.sqrt(2),
                torch.sum(torch.abs(Phi1_imag), dim=1).reshape(-1, 1) * np.sqrt(2),
                torch.sum(torch.abs(Phi2_real), dim=1).reshape(-1, 1) * np.sqrt(2),
                torch.sum(torch.abs(Phi2_imag), dim=1).reshape(-1, 1) * np.sqrt(2))
